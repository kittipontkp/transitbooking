...
// ดึงข้อมูลยานพาหนะจากฐานข้อมูล
    ...
    $car_result = $conn->query($sqlcar) or die($conn->error);
// ดึงข้อมูลผู้ขับขี่จากฐานข้อมูล
    $sqldriver = "SELECT * FROM drivers_list
                  WHERE `driver_status` = 'online'" ;
    $driver_result = $conn->query($sqldriver) or die($conn->error);


// นับจำนวนผู้ร่วมเดินทางมาแสดงใน accordion
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
          <h5 class="m-0 text-dark">จัดการข้อมูลยานพาหนะ | ผู้ขับขี่ยานพาหนะ</h5> <?php print_r($car_id); print_r($driver); ?>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="../dashboard">หน้าหลัก</a></li>
              <li class="breadcrumb-item"><a href="../articles">การจัดการข้อมูลยานพาหนะ</a></li>
                            ...
                            <tr>
                              <td>
                                <select class="form-control carselect2 " name="<?php echo 'cars' . $i ?>" style="width:100%;">
                                  <option value="">--เลือกยานพาหนะ--</option>
                                  <?php
                                  foreach ($car_result as $rowcar) {
                                    if ($cnt >= $i) {
                                      if ($rowcar['car_regis'] == $car_id[$j]) {
                                        echo "<option value='".$rowcar['car_regis']."' selected>".$rowcar['car_regis']."</option>";
                                      } else {
                                        echo "<option value='".$rowcar['car_regis']."'>".$rowcar['car_regis']."</option>";
                                      }
                                    }else{
                                      echo "<option value='".$rowcar['car_regis']."'>".$rowcar['car_regis']."</option>";
                                    }
                              ...
                              <td>
                                <select class="form-control driverselect2" name="<?php echo 'drivers' . $i ?>" style="width:100%;">
                                  <option value="">--คนขับยานพาหนะ--</option>
                                  <?php
                                  foreach ($driver_result as $rowdriver) {
                                    if ($cnt >= $i) {
                                      if ($rowdriver['name'] == $driver[$j]) {
                                        echo "<option value='".$rowdriver['name']."' selected>".$rowdriver['name']."</option>";
                                      } else {
                                        echo "<option value='".$rowdriver['name']."'>".$rowdriver['name']."</option>";
                                      }
                                    }else{
                                      echo "<option value='".$rowdriver['name']."'>".$rowdriver['name']."</option>";
                                    }
